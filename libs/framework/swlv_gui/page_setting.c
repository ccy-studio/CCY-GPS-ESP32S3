...
static const char* app_conf_sleep[4] = { "关闭","60秒","1小时","2小时" }; //自动休眠时间的设置
...
static void on_create_fun(ui_data_t* ui_dat, void* params) {
    ...
    lv_obj_set_style_bg_color(page_view, lv_color_hex(0xF5F5F5), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_color(text_title, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(text_title, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(text_title, &font_douyin_24, LV_PART_MAIN | LV_STATE_DEFAULT);

    //创建菜单列表
    menu = lv_menu_create(page_view);
    lv_obj_set_pos(menu, 0, 50);
    lv_obj_set_size(menu, lv_disp_get_hor_res(NULL), lv_disp_get_physical_ver_res(NULL) - 50);
    lv_obj_set_style_bg_color(menu, lv_color_hex(0xF5F5F5), LV_PART_MAIN);
    //去除Head按钮
    lv_obj_t* head_btn = lv_menu_get_main_header_back_btn(menu);
    ...
    lv_obj_set_scrollbar_mode(menu_main, LV_SCROLLBAR_MODE_OFF); //不显示滚动条
...
static void create_sub_page() {
    ...
    lv_obj_set_flex_align(sub_page_sleep, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_SPACE_BETWEEN);
    ...
    lv_obj_set_scrollbar_mode(sub_page_run_log, LV_SCROLLBAR_MODE_OFF); //不显示滚动条
    ...
    lv_obj_set_flex_align(sub_page_about, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    create_about_page();
}
...
static void update_gps_info_message(app_gps_t* gps) {
    ...
    lv_label_set_text_fmt(lv_obj_get_child(curr_speed, 0), "当前速度:%dkm/h", speed_kmh);
    lv_label_set_text_fmt(lv_obj_get_child(ant_state, 0), "天线状态:%s", gps->sats_in_use > 0 ? ANT_OK : ANT_FIL);
    lv_label_set_text_fmt(lv_obj_get_child(link_satellite_count, 0), "已连接卫星:%d个", gps->sats_in_use);
}
...
static lv_obj_t* create_run_log_item(app_run_log_t* log) {
    ...
    lv_obj_add_event_cb(plan, ui_callback_dynamic_scrollbar, LV_EVENT_FOCUSED, NULL);
    ...
    lv_obj_set_flex_align(row, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_SPACE_BETWEEN);
...
static lv_obj_t* create_select_item(const char* text, lv_obj_t* cont) {
    ...
    lv_obj_add_event_cb(btn, on_menu_item_selected_envent, LV_EVENT_FOCUSED, img);
    lv_obj_add_event_cb(btn, on_menu_item_selected_envent, LV_EVENT_DEFOCUSED, img);
...
static lv_obj_t* create_menu_item(const char* text, const char* icon_path) {
    ...
    lv_obj_add_event_cb(cont, on_menu_item_click_envent, LV_EVENT_CLICKED, text);
    lv_obj_add_event_cb(cont, on_menu_item_selected_envent, LV_EVENT_FOCUSED, img);
    lv_obj_add_event_cb(cont, on_menu_item_selected_envent, LV_EVENT_DEFOCUSED, img);
    lv_obj_set_style_radius(cont, 10, LV_STATE_DEFAULT | LV_PART_MAIN);
    lv_obj_set_style_bg_color(cont, lv_color_white(), LV_STATE_DEFAULT | LV_PART_MAIN);
...
static void on_menu_item_click_envent(lv_event_t* e) {
    ...
    if (item->user_data == sub_page_dail) {
        lv_obj_t* target = lv_obj_get_child(lv_obj_get_child(sub_page_dail, 0), app_store_get_conf_dial());
        set_focus(target);
    } else if (item->user_data == sub_page_sleep) {
        lv_obj_t* target = lv_obj_get_child(lv_obj_get_child(sub_page_sleep, 0), app_store_get_conf_sleep());
        set_focus(target);
    } else if (item->user_data == sub_page_run_log) {
...
static void on_menu_item_selected_envent(lv_event_t* e) {
    ...
    if (e->code == LV_EVENT_FOCUSED) {
        ...
        lv_obj_scroll_to_y(lv_obj_get_parent(screen), lv_obj_get_y(screen), LV_ANIM_ON);
...
static void on_start_fun(void* params) {
    //绑定订阅
    bus_button = bus_register_subscribe(DATA_BUS_BUTTON_EVENT, on_event_btn, NULL);
    bus_gps_info = bus_register_subscribe(DATA_BUS_GPS_REFRESH, on_event_gps_info, NULL);
}
...
ui_data_t page_setting = {
    .id = ACTIVITY_ID_SETTING,
